...
exports.driverRouter.get("/:id", (req, res) => {
  const driver = mock_data_1.driversDb.drivers.find(
...
exports.driverRouter.post("/", (req, res) => {
  const errors = (0, driver_body_validation_1.driverInputDtoValidation)(
    req.body,
  );
  if (errors.length > 0) {
  ...
  const newDriver = Object.assign(
    {
      id: mock_data_1.driversDb.drivers.length
        ? mock_data_1.driversDb.drivers[
            mock_data_1.driversDb.drivers.length - 1
          ].id + 1
        : 1,
